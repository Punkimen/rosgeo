	...
	$.fn.pagepiling = function (custom) {
		...
		function scrollPage(destination, animated) {
			var v = {
				...
				leavingSection:
					$(".pp-section.active").index(".pp-section") + 1,
			};
		...
		function isMoving() {
			...
			if (
				...
				scrollDelay + options.scrollingSpeed
			) {
				return true;
			}
			return false;
		...
		function removeMouseWheelHandler() {
			if (container.get(0).addEventListener) {
				...
				container
					.get(0)
					.removeEventListener("wheel", MouseWheelHandler, false); //Firefox
			} else {
		...
		function addMouseWheelHandler() {
			if (container.length) {
				if (container.get(0).addEventListener) {
					...
					container
						.get(0)
						.addEventListener("wheel", MouseWheelHandler, false); //Firefox
				} else {
					container
						.get(0)
						.attachEvent("onmousewheel", MouseWheelHandler); //IE 6/7/8
				}
			}
		}
