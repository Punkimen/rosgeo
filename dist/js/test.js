				...
				a.running ||
					...
					(a.instance.interval = setInterval(function () {
						a.instance.config.onBeforeStart.call(
							...
							setTimeout(function () {
								a.instance.config.infinite &&
									a.instance._overScroll(!0),
									a.instance.index <
									a.instance.pages.length - 1
										? a.instance.slideIndex++
										: (a.instance.slideIndex = 0),
									a.instance.scrollToIndex(
										a.instance.slideIndex,
									);
							}, a.config.delay || 0);
					}, a.config.interval)));
							...
							? void ((this.horizontal =
							  ...
							  this.pages.forEach(function (a, c) {
									...
									(d = e
										...
										: Array.isArray(f.config.anchors) &&
										  f.config.anchors.length
										? f.config.anchors[c]
												.replace(/\s+/, "-")
												.toLowerCase()
										: a.className
			...
			return (
					...
					if (!this.initialised && !this.container.pageable) {
						...
						if (
							...
							["Prev", "Next"].forEach(function (b) {
								...
								a[c] &&
									("string" == typeof a[c]
										? (this[c] = document.querySelector(
												a[c],
										  ))
										: _instanceof(a[c], Element) &&
										  (this[c] = a[c]),
				...
				(g.prototype.update = function () {
					...
					(this.wrapper.style["overflow-" + this.axis] = "scroll"),
						...
						(this.wrapper.style[b] =
							this.data.window[b] + this.bar + "px");
					...
					(this.container.style[a] = c * this.data.window[a] + "px"),
						(this.wrapper.style[
							"padding-" + (this.horizontal ? "bottom" : "right")
						] = this.bar + "px"),
						(this.wrapper[this.scrollAxis[this.axis]] =
							this.index * this.data.window[a] + d),
						(this.scrollSize =
							c * this.data.window[a] - this.data.window[a]),
						(this.scrollPosition =
					...
					if (this.initialised) {
						...
						["Prev", "Next"].forEach(function (a) {
							...
							this.config.slideshow &&
								(this.slider.stop(), (this.slider = !1)),
							(this.initialised = !1),
				...
				(g.prototype.off = function (a, b) {
					(this.listeners = this.listeners || {}),
						!1 == a in this.listeners ||
							this.listeners[a].splice(
								this.listeners[a].indexOf(b),
								1,
							);
				}),
				...
				(g.prototype._start = function (a) {
					...
					return (
						...
						("touchstart" !== a.type || this.events.touch
							...
							: (b.target.closest("a") || this._preventDefault(a),
							  !1))
					);
				}),
				(g.prototype._drag = function (a) {
					if (
						this.config.freeScroll &&
						this.dragging &&
						!this.scrolling
					) {
						var b = this._getEvent(a),
				...
				(g.prototype._stop = function (a) {
					...
					if (
						...
						return (
							...
							g &&
								(this.config.infinite &&
									this._overScroll(0 > h, h),
								0 < h ? e() : d()),
							this._scrollBy(
								this._getScrollAmount(this.oldIndex) - h,
							),
							void (this.down = !1)
						);
					}
					if (this.down && !this.scrolling) {
						var i =
							...
							j =
								...
								this.down[this.axis];
						g &&
							(this.config.infinite && this._overScroll(i),
							i ? d() : j && e()),
							this.startIndex === this.index
								? this.config.onFinish.call(
										this,
										this._getData(),
								  )
								: this._scrollBy(
										this._getScrollAmount(this.oldIndex),
								  ),
							(this.down = !1);
					}
				}),
					...
					if (
						...
						this.config.infinite && this._overScroll(d),
							d
								...
								: 0 < this.index && b--,
							b !== c &&
								((this.oldIndex = c), this.scrollToIndex(b));
					}
				}),
				...
				(g.prototype._getData = function () {
					var a = this.config.infinite
						...
						b = this.config.infinite
							? this.scrollSize -
							  2 * this.data.window[this.size[this.axis]]
							: this.scrollSize;
					return {
						index: this.index,
						percent: 100 * (a / b),
				...
				(g.prototype._overScroll = function (a) {
					...
					c === this.lastIndex && a
						? (c++,
						  this._scrollBy(
								-this.data.window[this.size[this.axis]] - b,
								c,
						  ))
						: 0 === c &&
						  !a &&
					...
					this.config.pips &&
						...
						this.pips.forEach(function (b, c) {
							c == a
								...
								: b.firstElementChild.classList.remove(
										"active",
								  );
						}));
				}),
